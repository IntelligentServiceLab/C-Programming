...
**【程序填空】功能：输出Fibonacci数列的前15项，要求每行输出5项。Fibonacci数列：1,1,2,3,5,8,13...........**
...
**【程序设计】题目：下面的公式可以用来计算圆周率PI的近似值: pi/8=1/（1\*3）+1/（5\*7）+1/（9\*11）+…… 请编程序计算公式的前15项，并输出pi值。**
...
```
**【程序设计】请编写函数sum，其功能是：用for循环语句求1到n之间所有偶数之和（若n为偶数包括n），并将结果返回给主函数。（n值由主函数传入）
...
**【程序设计】题目：编写函数fun，w是一个大于10的无符号整数，若w是n(n≥2)位的整数，则函数求出w的后n－1位的数作为函数值返回。
